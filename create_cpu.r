...
all.cpu <- read.csv("raw/cpu.csv", sep = ",")
all.cpu <- remove.factors(all.cpu)

# ajusta para nomes mais apropriados (bayes, linear, kmeans)
all.cpu$algo[which(all.cpu$algo == "bayes")] <- "nb"
all.cpu$algo[which(all.cpu$algo == "kmeans")] <- "kpar"
all.cpu$algo[which(all.cpu$algo == "linear")] <- "linr"
all.cpu$algo[which(all.cpu$algo == "lr")] <- "logr"


# ajusta para nomes mais apropriados (B1, B1, G, H, L)
exp1 <- all.cpu[all.cpu$exp == "exp1",]
exp1$scale <- "B1"

exp2 <- all.cpu[all.cpu$exp == "exp2",]
...
for (scale in scales){ 
    ...
    z <- exps[exps$scale == scale, c("algo", "hostname", "timestamp", "user", "system", "iowait", "softirq", "idle")]
    ...
    for (algo in algos) {
      ...
      write.csv(x, file = paste(algo, "_cpu_", scale, ".csv", sep = ""), row.names = F, quote = F)
